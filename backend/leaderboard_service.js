...
app.post('/add-leaderboard', (req, res) => {
    ...
    const leaderboard = { id, username, total_savings, parking_sessions };
    ...
    db.query(`INSERT INTO leaderboard SET ?`, leaderboard, (err, results) => {
        ...
        return res.status(200).json({ message: 'Leaderboard entry added successfully!' });
    });
});
...
app.get('/get-leaderboard/:username', (req, res) => {
    ...
    db.query(query, [username], (err, results) => {
        if (err) {
            console.error('[Database Error] Failed to retrieve leaderboard entry', err);
...
app.put('/edit-leaderboard/:username', (req, res) => {
    ...
    if (total_savings === undefined || parking_sessions === undefined) {
        return res.status(400).json({ error: 'Both total_savings and parking_sessions are required' });
    }
    ...
    db.query(query, [total_savings, parking_sessions, username], (err, results) => {
        if (err) {
            console.error('[Database Error] Failed to update leaderboard entry', err);
        ...
        return res.status(200).json({ message: 'Leaderboard entry updated successfully' });
    });
});
