...
export default function Stepper({
  ...
  const handleNext = async () => {
    ...
    if (onNextStep) {
      const canProceed = await onNextStep(currentStep);
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
  ...
  return (
    <motion.div 
      ...
      initial={false}
      style={{ 
        cursor: disableStepIndicators ? 'default' : (step <= currentStep ? 'pointer' : 'not-allowed'),
        opacity: disableStepIndicators && step !== currentStep ? 0.6 : 1
      }}
    >
      <motion.div
        variants={{
